...
class WorkflowStateTests(APITestCase):
    ...
    def test_WF_02_tidak_bisa_edit_laporan_yang_sudah_reported(self):
        ...
        payload = {
            'title': 'Saluran Air Tersumbat - Update',
            ...
            'status': 'REPORTED',  # Tetap REPORTED, tapi mencoba ubah
        }

        response = self.client.put(url, payload, format='json')

        # Verifikasi: PUT ditolak dengan HTTP 403 Forbidden
        self.assertEqual(
            response.status_code,
    ...
    def test_WF_05_laporan_resolved_tidak_bisa_diubah(self):
        ...
        payload = {
            ...
            'status': 'RESOLVED',  # Tetap RESOLVED, tapi mencoba ubah
        }
        ...
        self.assertEqual(
            response.status_code,
            status.HTTP_403_FORBIDDEN,
            "Laporan RESOLVED seharusnya read-only; modifikasi ditolak (HTTP 403)"
        )
